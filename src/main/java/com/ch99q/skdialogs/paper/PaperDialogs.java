...
public final class PaperDialogs {
    ...
    private static DialogBody buildBody(DialogSpec.Body b) {
        if (b.kind == DialogSpec.Body.Kind.ITEM) {
            return DialogBody.item(b.item).build();
        }
        ...
        switch (in.kind) {
            case TEXT -> {
                ...
                if (Boolean.TRUE.equals(in.multiline)) {
                    builder.multiline(TextDialogInput.MultilineOptions.create(null, null));
                }
                return builder.build();
            }
            case BOOLEAN -> {
            ...
            case SLIDER -> {
                ...
                if (in.sliderDefault != null) builder.initial(in.sliderDefault);
                return builder.build();
            }
            case DROPDOWN -> {
                ...
                return DialogInput.singleOption(in.key, in.label, entries).build();
            }
