...
@Examples({
        ...
        "add text input \"note\" labeled \"Note\" multiline",
...
public class EffInput extends DialogEffect {
    ...
    protected void apply(DialogSpec spec, Event event) {
        String key = string(0, event);
        if (key == null) {
            return;
        }
        Input input = new Input(kindOf(), key);
        String label = string(1, event);
        if (label != null) {
            input.label = Text.component(label);
        }
        switch (kind) {
            case TEXT -> {
                input.textDefault = string(2, event);
            ...
            case SLIDER -> {
                ...
                Number max = number(3, event);
                Number step = number(4, event);
                Number def = number(5, event);
    ...
    private @Nullable Object single(int i, Event event) {
